...
RdpClientRose::RdpClientRose(base::Thread& thread, rtc::MessageHandler& message_handler, twebrtc_send_helper& send_helper)
	: thread_(thread)
...
{
	VALIDATE_IN_RDPC_THREAD();
	tauto_destruct_executor destruct_executor(boost::bind(&RdpClientRose::did_slice_quited, this, timeout));
...
void trdp_manager::start_internal(uint32_t ipaddr, int port, void* decoder, void* desktop, rtc::MessageHandler* message_handler_ptr, twebrtc_send_helper* send_helper_ptr)
{
	...
	{
		tauto_destruct_executor destruct_executor(boost::bind(&trdp_manager::did_set_event, this));
		delegate_.reset(new RdpClientRose(*thread_.get(), *message_handler_ptr, *send_helper_ptr));
	}
