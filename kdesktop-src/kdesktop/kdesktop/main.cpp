...
static int do_gameloop(int argc, char** argv)
{
	...
	try {
		...
		for (; ;) {
			...
			{
				// gui2::thome dlg(game.ble());
				gui2::thome dlg(game.pbremotes());
				dlg.show();
				res = static_cast<gui2::thome::tresult>(dlg.get_retval());
				rdpcookie = dlg.rdpcookie();
			}
			if (res == gui2::thome::SCAN) {
				gui2::tscan dlg(game.ble());
				dlg.show();
				// res = static_cast<gui2::tscan::tresult>(dlg.get_retval());

			} else if (res == gui2::thome::DESKTOP) {
